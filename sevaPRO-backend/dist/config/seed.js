...
async function seedDatabase() {
    ...
    if (serviceCount === 0) {
        ...
        await Service_1.Service.insertMany(servicesToInsert);
        console.log('✅ Seeded services');
    }
    const adminExists = await User_1.User.findOne({ email: 'admin@seva.com' });
    if (!adminExists) {
        await User_1.User.create({ name: 'Admin', email: 'admin@seva.com', password: 'Admin@123', role: 'admin' });
        console.log('✅ Seeded admin user');
    }
    const providerExists = await User_1.User.findOne({ email: 'provider@seva.com' });
    if (!providerExists) {
        await User_1.User.create({ name: 'Rohit Sharma', email: 'provider@seva.com', password: 'Provider@123', role: 'provider', phone: '+919876543210', rating: 4.8, totalJobs: 124, profileImage: 'https://i.pravatar.cc/150?u=rohit' });
